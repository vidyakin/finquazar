<template>
  <div class="main">
    ...
    <div class="row">
      <div class="col filestring">Файл ведомости: <span class="filename">{{filename}}</span></div>
    </div>

    <div class="row justify-start">
      <div class="col1">
        <q-btn color="secondary" icon="attach_file" @click="chooseFile()" label="Загрузить ведомость" />
      </div>

      <div class="col1">
        <q-select label="Тип формы" stack-label filled 
          v-model="currForm" 
          :options="forms"
          option-value="id"
          option-label="desc"
      ...
      <div class="col1">
        <q-select label="Счет:" stack-label filled
          v-model="currAcc" 
          :options="accs"
  ...
  methods: {
    chooseFile() {
      let opt = {
        ...
        filters: [
          {name: 'Excel файлы', extensions: ['xls', 'xlsx']}
        ]
      }
...
body { font-family: 'Open Sans Condenced', sans-serif !important }
